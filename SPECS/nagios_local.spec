...
%define _localstatedir /usr/local
%define _sysconfdir /usr/local
...
CFLAGS="%{mycflags} %{myXcflags}" LDFLAGS="$CFLAGS" %configure \
    ...
    --with-checkresult-dir="%{_localstatedir}/nagios/var/spool/checkresults" \
...
if ! /usr/bin/id nagios &>/dev/null; then
    /usr/sbin/useradd -r -d %{logdir} -s /bin/sh -c "nagios" --uid 3001 --gid 3001 nagios || \
        %logmsg "Unexpected error adding user \"nagios\". Aborting installation."
fi
if ! /usr/bin/getent group nagcmd &>/dev/null; then
    /usr/sbin/groupadd nagcmd --gid 3002 &>/dev/null || \
        %logmsg "Unexpected error adding group \"nagcmd\". Aborting installation."
fi
...
#         /usr/sbin/usermod -a -G nagios,nagcmd apache &>/dev/null
#     fi
# else
